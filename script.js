...
async function addPending(cid, price, title="Item") {
  const c = (await getAllCustomers()).find(c=>c.id===cid); if(!c) return;
...
async function showAllStatements() {
  ...
  customers.forEach(c=>{
    ...
    div.innerHTML+=`<div style="margin:1em 0;padding:1em;border:1px solid #444;border-radius:6px;">
      <strong>${c.name}</strong><br>Purchases: $${totalPurchases(c).toFixed(2)}<br>Payments: $${totalPayments(c).toFixed(2)}<br><strong>${balText}</strong>
      <h4>Purchases</h4><ul>${purchaseList}</ul><h4>Payments</h4><ul>${paymentList}</ul>
    </div>`;
  });
}
...
async function render() {
  const div=document.getElementById("customers"); div.innerHTML="";
  ...
  customers.filter(c=>c.name.toLowerCase().includes(search)).forEach(c=>{
    ...
    const bal=balance(c); const balClass=bal<0?"credit":(bal>0?"debt":""); const balText=bal<0?`In Credit: $${Math.abs(bal).toFixed(2)}`:`Owing: $${bal.toFixed(2)}`;
    let pendingList=c.pending.map((p,i)=>`<li>${p.title} ×${p.qty} — $${subtotal(p).toFixed(2)} <small>(${p.date}, ${p.server})</small>
    ...
    el.innerHTML=`
      ...
      <h4>Pending Orders</h4><ul>${pendingList}</ul>
